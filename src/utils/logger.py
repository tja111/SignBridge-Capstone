...
class SignLanguageLogger:
    ...
    def __init__(self, name: str = "sign_language", level: str = "INFO"):
        ...
        # A Windows installer may place the executable under Program Files,
        # which is not writable. Keep development logs local while allowing
        # packaged inference to start without administrator permissions.
        if getattr(sys, "frozen", False):
            self.logs_dir = Path(os.environ.get("LOCALAPPDATA", Path.cwd())) / "SignBridge" / "logs"
        else:
