...
class DETRData(Dataset): 
    def __init__(self, path, train=True, words_mode=False, words_image_size=224):
        super().__init__()
        self.path = path
        ...
        self.words_image_size = words_image_size
        ...
        if self.words_mode:
            transform_list = [
                f"Resize to {self.words_image_size}x{self.words_image_size} (matches webcam inference)",
                *(["Mild horizontal flip and brightness/contrast jitter (training only)"] if train else []),
                "Normalize (ImageNet stats)", "Convert to Tensor",
            ]
        else:
            transform_list = [
                "Resize to 500x500", "Random Crop 224x224 (training only)",
                "Final Resize to 224x224", "Horizontal Flip p=0.5 (training only)",
                "Color Jitter (training only)", "Normalize (ImageNet stats)",
                "Convert to Tensor",
            ]
        self.data_handler.log_transform_info(transform_list)             

        if self.words_mode:
            # Word-level signs often require both hands and body/face context.
            # The old 500px -> random 224px crop discarded that context in one
            # third of samples, while webcam inference always receives the full
            # frame. Keep this geometry consistent with inference and use only
            # mild appearance changes that reflect webcam variation.
            train_transforms = []
            if self.train:
                train_transforms = [
                    A.HorizontalFlip(p=0.3),
                    A.RandomBrightnessContrast(
                        brightness_limit=0.18, contrast_limit=0.18, p=0.4
                    ),
                    A.HueSaturationValue(
                        hue_shift_limit=8, sat_shift_limit=12,
                        val_shift_limit=8, p=0.25
                    ),
                ]
            transforms = [A.Resize(self.words_image_size, self.words_image_size), *train_transforms]
        else:
            # Preserve the existing alphabet augmentation behaviour.
            transforms = [
                A.Resize(500, 500),
                ...
                *([A.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5,
                                 hue=0.5, p=0.5)] if self.train else []),
            ]
    ...
    def __getstate__(self):
        ...
        state["logger"] = None
        state["data_handler"] = None
        return state

    def __setstate__(self, state):
        self.__dict__.update(state)
        if self.logger is None or self.data_handler is None:
            self.logger = get_logger("data_loader")
            self.data_handler = DataLoaderHandler()
    ...
    def safe_transform(self, image, bboxes, labels, max_attempts=50):
        ...
        # The transform above should preserve every box.  Raise an actionable
        # error instead of returning an unnormalised NumPy image that later
        # fails inside the model with an unrelated tensor error.
        raise RuntimeError("Could not transform a sample while preserving its labels")
